...
#else
// Full step table (emits events at final states only)
...
Rotary::Rotary(uint8_t _pin1, uint8_t _pin2) {
    ...
    port1 = digitalPinToPort(pin1);
    port2 = digitalPinToPort(pin2);

    // Get pointers to the input registers of the ports
    pinReg1 = portInputRegister(port1);
    pinReg2 = portInputRegister(port2);
